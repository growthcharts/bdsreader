...
test_that("handles the empty individual object", {
  expect_message(
    read_bds(js1, schema = "bds_schema_str.json"),
...
test_that("test1.json (client3.json) passes read_bds()", {
  expect_equal(
    class(read_bds(jtf[1], schema = "bds_schema_str.json")),
...
test_that("test2.json (missing Referentie) PASSES", {
  expect_equal(
    class(read_bds(jtf[2], schema = "bds_schema_str.json")),
    c("tbl_df", "tbl", "data.frame")
  )
})
...
test_that("test3.json (missing OrganisatieCode) MESS", {
  expect_message(
    read_bds(jtf[3], schema = "bds_schema_str.json"),
...
test_that("test4.json (wrong type) MESS", {
  expect_message(
    read_bds(jtf[4], schema = "bds_schema_str.json"),
...
test_that("test5.json (missing ClientGegevens) MESS", {
  expect_message(
    read_bds(jtf[5], schema = "bds_schema_str.json"),
    "should have required property 'ClientGegevens'"
  )
})

test_that("test6.json (Missing ContactMomenten) MESS", {
  expect_message(read_bds(jtf[6], schema = "bds_schema_str.json"),
...
test_that("test7.json (Missing Referentie & OrganisatieCode) MESS", {
  expect_message(
    read_bds(jtf[7], schema = "bds_schema_str.json"),
    "should have required property 'OrganisatieCode'"
  )
})
...
test_that("test8.json (Invalid JSON) ERROR", {
  expect_error(
    read_bds(jtf[8], schema = "bds_schema_str.json"),
...
test_that("test9.json (Bdsnummer 19 missing) MESS", {
  expect_message(
    read_bds(jtf[9], schema = "bds_schema_str.json"),
...
test_that("test10.json (Bdsnummer 20 missing) MESS", {
  expect_message(
    read_bds(jtf[10], schema = "bds_schema_str.json"),
    "verplicht BDS nummer ontbreekt: 20"
  )
})

test_that("test11.json (Bdsnummer 82 missing) MESS", {
  expect_message(read_bds(jtf[11], schema = "bds_schema_str.json"),
...
test_that("test12.json (Bdsnummer 91 missing) PASSES", {
  expect_silent(read_bds(jtf[12], schema = "bds_schema_str.json"))
})

test_that("test13.json (Bdsnummer 110 missing) MESS", {
  expect_message(read_bds(jtf[13], schema = "bds_schema_str.json"),
...
test_that("test14.json (empty file) ERROR", {
  expect_error(read_bds(jtf[14], schema = "bds_schema_str.json"), "premature EOF")
})

test_that("test15.json (Bdsnummer 62 numeric) MESS", {
  expect_message(read_bds(jtf[15], schema = "bds_schema_str.json"))
})

test_that("test16.json (Bdsnummer 20 numeric) PASSES", {
  expect_silent(read_bds(jtf[16], schema = "bds_schema_str.json"))
})

test_that("test17.json (Bdsnummer 82 numeric) PASSES", {
  expect_silent(read_bds(jtf[17], schema = "bds_schema_str.json"))
})

test_that("test18.json (Bdsnummer 91 numeric) MESS", {
  expect_message(
    read_bds(jtf[18], schema = "bds_schema_str.json"),
...
test_that("test19.json (Bdsnummer 110 numeric) PASSES", {
  expect_silent(read_bds(jtf[19], schema = "bds_schema_str.json"))
})

test_that("test20.json (missing Groepen) MESS", {
  expect_message(read_bds(jtf[20], schema = "bds_schema_str.json"),
...
test_that("test21.json (minimal data) MESS", {
  expect_message(read_bds(jtf[21], schema = "bds_schema_str.json"),
    "Missing 'Contactmomenten'",
    fixed = TRUE
  )
})
...
test_that("test22.json (gad (=49) out of range 50-350: set to NA)", {
  expect_message(read_bds(jtf[22], schema = "bds_schema_str.json"))
})

test_that("test23.json (multiple messages) MESS", {
  expect_message(read_bds(jtf[23], schema = "bds_schema_str.json"))
})

test_that("test24.json (new DDI fields) SILENT", {
  expect_silent(read_bds(jtf[24], schema = "bds_schema_str.json"))
})
...
test_that("Laura_S.json is silent with GA in days", {
  expect_silent(read_bds(js, schema = "bds_schema_str.json"))
})
...
test_that("not_a_vector.json produces messages", {
  expect_message(read_bds(js, schema = "bds_schema_str.json"),
    "BDS 82",
    fixed = TRUE
  )
})
