convert_ddi_gsed_3 <- function(bds) {
  ...
  if (!hasName(bds, "category")) {
    w <- tibble(lex_gsed = character(0), age = numeric(0), pass = integer(0))
    return(w)
  }
  ...
  w1 <- left_join(ddi, itm, by = "bds", relationship = "many-to-many") %>%
  ...
  w[
    (w$age < 0.5 / 365.25 | w$age > 42.5 / 365.25) &
      w$lex_gsed == "ddigmd155",
    "pass"
  ] <- NA_real_
  w[
    (w$age < 42.5 / 365.25 | w$age > 102.5 / 365.25) &
      w$lex_gsed == "ddigmd255",
    "pass"
  ] <- NA_real_
  w[
    (w$age < 102.5 / 365.25 | w$age > 146.5 / 365.25) &
      w$lex_gsed == "ddigmd355",
    "pass"
  ] <- NA_real_
  w[
    (w$age < 146.5 / 365.25 | w$age > 1) &
      w$lex_gsed == "ddigmd055",
    "pass"
  ] <- NA_real_

  w[
    (w$age < 0.75 | w$age >= 1.75) &
      w$lex_gsed == "ddigmd068",
    "pass"
  ] <- NA_real_
  w[
    (w$age < 1.75 | w$age >= 2.50) &
      w$lex_gsed == "ddigmd168",
    "pass"
  ] <- NA_real_
  w[
    (w$age < 2.5 | w$age >= 3.50) &
      w$lex_gsed == "ddigmd268",
    "pass"
  ] <- NA_real_
...
convert_var_gsed_3 <- function(raw) {
  ...
  dob <- filter(raw$clientDetails, .data$bdsNumber == 20L) %>%
    pull("value") %>%
  ...
  w <- raw$clientMeasurements %>%
    ...
    transmute(
      ...
      age = if ("date" %in% names(.)) {
        round(as.numeric(as.Date(date, format = "%Y%m%d") - dob) / 365.25, 4)
      } else {
        NA_real_
      },
      pass = if ("value" %in% names(.)) as.numeric(.data$value) else NA_real_
    )
  return(w)
}
